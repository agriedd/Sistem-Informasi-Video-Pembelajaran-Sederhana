...
try {
	$query_tambah_video = "DELETE FROM video_pembelajaran WHERE id_video=:id_video LIMIT 1";
...
START TRANSACTION;
DELETE FROM <code class="text-green-400">video_pembelajaran</code> WHERE <code class="text-pink-400">id_video</code>=<code class="text-blue-400"><?=$_GET['id_video'] ?></code> LIMIT 1;
COMMIT;</pre>
						</div>
					</div>
					<?php 
							...
							<div class="p-1 rounded-full bg-slate-100">
								<ul class="flex gap-1">
									<li class="transition-all duration-75 px-4 py-2 rounded-full text-slate-600 hover:text-slate-400">
										<a href="/admin/kelas/info-kelas.php?id_kelas=<?=$_GET['id_kelas'] ?>" class="text-sm">
											Kembali
										</a>
									</li>
								</ul>
