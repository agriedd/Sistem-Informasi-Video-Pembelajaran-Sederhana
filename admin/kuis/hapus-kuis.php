...
try {
	$query_tambah_kuis = "DELETE FROM kuis WHERE id_kuis=:id_kuis LIMIT 1";
...
START TRANSACTION;
DELETE FROM <code class="text-green-400">kuis</code> WHERE <code class="text-pink-400">id_kuis</code>=<code class="text-blue-400"><?=$_GET['id_kuis'] ?></code> LIMIT 1;
COMMIT;</pre>
						</div>
					</div>
					<?php 
							...
							<div class="p-1 rounded-full bg-slate-100">
								<ul class="flex gap-1">
									<li class="transition-all duration-75 px-4 py-2 rounded-full text-slate-600 hover:text-slate-400">
										<a href="/admin/kuis/info-video.php?id_video=<?=$_GET['id_video'] ?>" class="text-sm">
											Kembali
										</a>
									</li>
								</ul>
