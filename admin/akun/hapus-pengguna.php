...
try {
	$query_tambah_pengguna = "DELETE FROM pengguna WHERE id_pengguna=:id_pengguna LIMIT 1";
	$query = $sambungan->prepare($query_tambah_pengguna);
	$hasil = $query->execute([
...
START TRANSACTION;
DELETE FROM <code class="text-green-400">pengguna</code> WHERE <code class="text-pink-400">id_pengguna</code>=<code class="text-blue-400"><?=$_GET['id_pengguna'] ?></code> LIMIT 1;
COMMIT;</pre>
						</div>
					</div>
					<?php 
